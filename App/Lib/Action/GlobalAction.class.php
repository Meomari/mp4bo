...
class GlobalAction extends Action
{
    ...
    function _initialize()
    {
      ...
      // print_r($this->globalCategory);
	    ...
	    if(MODULE_NAME=="Index") $menu=1;
		if(MODULE_NAME=="product") $menu=2;
		if(MODULE_NAME=="Page" && $_REQUEST['item']=="Experience")  $menu=3;
		if(MODULE_NAME=="Page" && $_REQUEST['item']=="by")  $menu=4;
if(MODULE_NAME=="Page" && $_REQUEST['item']=="pay")  $menu=4;
if(MODULE_NAME=="Page" && $_REQUEST['item']=="Help")  $menu=5;
if(MODULE_NAME=="Page" && $_REQUEST['item']=="HelpList" || MODULE_NAME== "Article")  $menu=5;
if(MODULE_NAME=="Page" && $_REQUEST['item']=="Service")  $menu=6;
if(MODULE_NAME=="Page" && $_REQUEST['item']=="Cases")  $menu=7;
    ...
    {   
        ...
        $listRows = empty($listRows) ? 1 : $listRows;
        $orderd = empty($orders) ? 'id DESC' : $orders;
        $paged = new page($pageCount, $listRows);
    ...
    public function getJoinDetail($conditions = '', $viewCount = false, $table = '', $join = '', $fields = '')
    {
		...
		$viewCount && $this->dao->Where($condition2)->setInc($viewCount,1);
		  ...
		  if($contentDetail['video_addr'] !=""){
			    
				$newarr=array();
			   $arr=explode(',',$contentDetail['video_addr']);
			    
				for($i=0;$i<count($arr);$i++){
					
					$newarr[]=array('url'=>$arr[$i],'title'=>'片段'.($i+1)."");
					
					
					}
			
			  }
		  $contentDetail['video_addr']="var arr=".json_encode($newarr).";";
     ...
     */
	public function getDingcai($id,$dingcai,$mod){
		   ...
		   if(Cookie::get("$zifu")==""){
                 ...
                 $di=$this->dao->Where('id='.$id)->setInc($dingcai,1);
				 
				 if($dingcai=='ding' && Cookie::get($mod.'cai'.$id) ==true )  $di=$this->dao->Where('id='.$id)->setInc('cai',-1);
				 if($dingcai=='cai' && Cookie::get($mod.'ding'.$id) ==true )  $di=$this->dao->Where('id='.$id)->setInc('ding',-1);
				 
                 $dc=$this->dao->Where('id='.$id)->find();
		         $dx['ding']=$dc['ding'];
			     $dx['cai']=$dc['cai']; 
				 Cookie::set("$zifu",session_id().$zifu.$id,60*60*12);
