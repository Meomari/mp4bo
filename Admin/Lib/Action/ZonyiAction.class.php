 ...
 * @version        	$Id: ZyAction.class.php v2.0 2010-01-01 06:59:03 

 */

class ZyAction extends GlobalAction
{
    ...
    public function index()
    {
        ...
        $recommend = inZyal($_GET['recommend']);
        $categoryId = inZyal($_GET['categoryId']);
        $userId = inZyal($_GET['userId']);
        $status =  inZyal($_GET['status']);
        $istop = inZyal($_GET['istop']);
        $createTime = trim($_GET['createTime']);
        $createTime1 = trim($_GET['createTime1']);
        $viewCount = inZyal($_GET['viewCount']);
        $viewCount1 = inZyal($_GET['viewCount1']);
        ...
        $dataList = $this->dao->Table(C('DB_PREFIX').'Zy a')->Join(C('DB_PREFIX').'category b on a.category_id=b.id')->Field('a.*,b.title as category')->Order($setOrder)->Where($condition)->Limit($p->firstRow.','.$p->listRows)->select();
    ...
    {   global $rr;
        parent::_checkPermission('Zy_insert');
    ...
    {
        parent::_checkPermission('Zy_modify');
        parent::_setMethod('post');
        $item = inZyal($_POST['id']);
    ...
    public function doCommand()
    {
        ...
        parent::_checkPermission('Zy_command');
        if(getMethod() == 'get'){
        ...
        $newCategory = inZyal($_POST['newCategory']);
		
        switch ($operate){
            case 'delete':
                parent::_tagsDelete('Zy');
